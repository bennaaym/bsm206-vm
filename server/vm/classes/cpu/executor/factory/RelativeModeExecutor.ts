...
class RelativeModeExecutor implements IExecutor
{
    ...
    private BVC = (): void =>
    {
        // T3 if V == 0 : PC <- effective address
        const overflowFlag = this.registersRef.getRegister(EREG.CCR).getFlag(EFLAG.V);
        if(!overflowFlag) this.assignEffectiveAddressToPC();
    }
